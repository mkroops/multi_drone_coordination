...
print("state: %s" % s)
state1 = client.getMultirotorState(vehicle_name="Drone3")
s = pprint.pformat(state1)
print("state: %s" % s)
state2 = client.getMultirotorState(vehicle_name="Drone4")
s = pprint.pformat(state2)
print("state: %s" % s)
state1 = client.getMultirotorState(vehicle_name="Drone5")
s = pprint.pformat(state1)
print("state: %s" % s)



# Class task, To assign task
class Task:
...
class Drone:
    ...
    #estimate best task and centrally allocate task to queen drone by k means clustering
    def estimate_best_task_for_queen(self, assigned_tasks):
        ...
        self.path.append(assigned_tasks[index])
...
#Auction class, central enity wiil distribute tasks
class Auction:
    ...
    #metric for calculte fairness score
    ...
    def metrics(self, drones):
        ...
        for drone in drones:
            if drone.is_am_queen == 'Queen':
                print("///////////Drone path", len(drone.path))
    ...
    def execute_drone_tasks(self):
        ...
        queen_count = 0
        queen_drone = None
        f1 = client.moveToPositionAsync(1, 0, -10, 10, vehicle_name="Drone1")
        f2 = client.moveToPositionAsync(2, 0, -10, 10, vehicle_name="Drone2")
        f3 = client.moveToPositionAsync(3, 0, -10, 10, vehicle_name="Drone3")
        f4 = client.moveToPositionAsync(4, 0, -10, 10, vehicle_name="Drone4")
        f5 = client.moveToPositionAsync(5, 0, -10, 10, vehicle_name="Drone5")
        f1.join()
        f2.join()
        f3.join()
        f4.join()
            ...
            for drone in self.drones:
                if drone.is_am_queen == 'Queen':
                    ...
                    if i >= len(drone.path):
                        ...
                        pos = drone.path[i]
                        print(drone.drone_id, "Positions", pos.positions)
                        k = 5
                        if drone.drone_id == 'drone2':
                            f2 = client.moveToPositionAsync(((pos.positions[0])*k), ((pos.positions[1])*k), -10, 10, vehicle_name="Drone2")

                        if drone.drone_id == 'drone3':
                            f3 = client.moveToPositionAsync(((pos.positions[0])*k), ((pos.positions[1])*k), -10, 10, vehicle_name="Drone3")

                        if drone.drone_id == 'drone4':
                            f4 = client.moveToPositionAsync(((pos.positions[0])*k), ((pos.positions[1])*k), -10, 10, vehicle_name="Drone4")

                        if drone.drone_id == 'drone5':
                            f5 = client.moveToPositionAsync(((pos.positions[0])*k), ((pos.positions[1])*k), -10, 10, vehicle_name="Drone5")
                        time.sleep(0.5)

                if queen_count <= 2 and drone.is_am_queen == 'Queen': 
                    k = 5
                    if i == 0:
                        pos = drone.path[queen_count]
                        f1 = client.moveToPositionAsync(((pos.positions[0])*k), ((pos.positions[1])*k), -10, 10, vehicle_name="Drone1")
                        queen_count = queen_count + 1

                    if count == int(0.3 * len(self.tasks)):
                        pos = drone.path[queen_count]
                        queen_count = queen_count + 1
                        f1 = client.moveToPositionAsync(((pos.positions[0])*k), ((pos.positions[1])*k), -10, 10, vehicle_name="Drone1")
                        count = 0
